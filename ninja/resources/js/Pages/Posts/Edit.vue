...
const form = useForm({
	...
	content: usePage().props.value.post.content,
	slug: usePage().props.value.post.slug,
	user: usePage().props.value.user.id,
	category_id: postCategory.value.id,
});
...
const submit = () => {
	console.log(postCategory.value.id)
	form.put(route('posts.update', usePage().props.value.post), {
		errorBag: 'updatePost',
		preserveScroll: true,
			...
			<div class="col-span-2">
				<form class="grid gap-7" @submit.prevent="submit">
					...
					<multiselect 
						...
						:options="usePage().props.value.categories"
						label="name"
						/>
